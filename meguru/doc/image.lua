...
--
-- The render is asked to produce a buffer whose long edge is at most the
-- configured cap, so everything downstream — the native LRU (up to 3 pages),
-- the whole-page crop intermediates, and the pan/zoom sub-rectangle copies —
-- never holds a multi-thousand-pixel buffer either. The cap therefore only
-- bounds *retained* resolution (and RAM); the decode peak itself is set by
-- MuPDF's subsampling, not by the cap. Lossless formats (PNG, GIF, ...) are
-- still decoded at full size inside MuPDF before being subsampled — the same
-- one-transient profile the old path had, never worse.
--
-- Pages at or under the cap come back at their natural resolution, so
-- normal-sized books are completely unaffected. The cap comes from the
-- `max_native_dim` preference.

local function cappedDim(w, h, cap)
...
local function renderMuPDFPage(doc, pageno, refuse_oversize)
    local cap = Settings.get("max_native_dim")
    ...
    if ok_page and page then
        ...
        if ok_size and pw and ph and pw > 0 and ph > 0 then
            ...
            local cw, ch = cappedDim(fw, fh, cap)
            ...
            if ok_draw and rendered then
                ...
                logger.dbg(string.format(
                    "Meguru: MuPDF page render %dx%d -> %dx%d (cap %d)",
                    fw, fh, cw, ch, cap))
            else
...
local function decodeNativeRenderImage(data)
    ...
    local cw, ch = cappedDim(w, h, cap)
    if cw == w and ch == h then
        return bb -- at or under the cap: return the decode untouched
    end
    logger.dbg(string.format(
        "Meguru: page decode %dx%d over the %dpx cap, downscaling to %dx%d",
        w, h, cap, cw, ch))
